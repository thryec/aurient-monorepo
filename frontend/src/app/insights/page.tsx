...
import { healthDataService, HealthAnalysisResponse, StructuredDailyAdvice, DailyCard } from '@/lib/healthDataService';

export default function InsightsPage() {
  const [loading, setLoading] = useState(false);
  const [dailyAnalysis, setDailyAnalysis] = useState<HealthAnalysisResponse | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [showDailyInsights, setShowDailyInsights] = useState(false);

  useEffect(() => {
    // Apply background to body when component mounts
    document.body.style.background = 'linear-gradient(to bottom right, rgb(254 240 138), rgb(251 207 232), rgb(255 237 213), rgb(252 165 165))';
          ...
          {showDailyInsights && dailyAnalysis?.success && dailyAnalysis.data ? (
            ...
            <div className="w-full max-w-4xl">
              <div className="space-y-6">
                {dailyAnalysis.data.structured_advice && 'cards' in dailyAnalysis.data.structured_advice ? (
                  renderAIGeneratedCards(dailyAnalysis.data.structured_advice as StructuredDailyAdvice)
                ) : (
